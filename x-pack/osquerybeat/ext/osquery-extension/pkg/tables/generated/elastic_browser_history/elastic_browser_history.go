...
type Result struct {
	Timestamp         time.Time `osquery:"timestamp" format:"unix"`            // Unix timestamp in seconds since epoch (visit time)
	Datetime          time.Time `osquery:"datetime" format:"rfc3339" tz:"UTC"` // Human-readable datetime string in RFC3339 format (visit time)
	UrlId             int64     `osquery:"url_id"`                             // Unique URL identifier within the browser profile
	Scheme            string    `osquery:"scheme"`                             // URL scheme/protocol (e.g., "https", "http", "file", "ftp")
	Domain            string    `osquery:"domain"`                             // Registrable domain (eTLD+1) extracted from hostname (e.g., "github.com", "google.com", "example.co.uk")
	Hostname          string    `osquery:"hostname"`                           // Full hostname from URL (e.g., "www.github.com", "mail.google.com")
	...
	User              string    `osquery:"user"`                               // Username/profile owner
	ProfileName       string    `osquery:"profile_name"`                       // Browser profile name
	TransitionType    string    `osquery:"transition_type"`                    // Navigation method - how user reached this page (TYPED, LINK, BOOKMARK, RELOAD, REDIRECT, etc.)
	ReferringUrl      string    `osquery:"referring_url"`                      // URL that linked to this page (if navigated via link)
	...
	VisitSource       string    `osquery:"visit_source"`                       // Data origin - where this visit data came from (browsed/local, synced, imported, extension)
	...
	ChVisitDurationMs int64     `osquery:"ch_visit_duration_ms"`               // Duration of visit in milliseconds (Chromium-based browsers only)
	FfSessionId       int32     `osquery:"ff_session_id"`                      // Firefox session tracking identifier
	FfFrecency        int32     `osquery:"ff_frecency"`                        // Firefox frecency score (frequency + recency algorithm)
	SfDomainExpansion string    `osquery:"sf_domain_expansion"`                // Safari domain classification/expansion
	SfLoadSuccessful  bool      `osquery:"sf_load_successful"`                 // Whether page loaded successfully (1) or failed (0)
	CustomDataDir     string    `osquery:"custom_data_dir"`                    // Custom data directory path (for querying non-standard locations)
}
